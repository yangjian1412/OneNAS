...
import { MoreButton, Checkbox } from './navidromeRowIcons'
import Icon from '@/components/Icon'

export interface SongRowCallbacks {
  onMorePress?: (song: NavidromeSong, index: number) => void
  onLongPress?: (song: NavidromeSong, index: number) => void
  onToggleSelect?: (song: NavidromeSong, index: number) => void
  isSelected?: (song: NavidromeSong, index: number) => boolean
  multiSelect?: boolean
}

interface Props extends SongRowCallbacks {
  songs: NavidromeSong[]
...
export default function NavidromeSongList({
  songs, onSongPress, emptyText = '暂无歌曲',
  onMorePress, onLongPress, onToggleSelect, isSelected, multiSelect,
}: Props) {
  const t = useTheme()
  ...
  return (
    <FlatList
      ...
      renderItem={({ item, index }) => {
        const selected = isSelected ? isSelected(item, index) : false
        return (
          <TouchableOpacity
            style={[styles.row, { borderBottomColor: t.border }]}
            activeOpacity={0.7}
            onPress={() => {
              if (multiSelect && onToggleSelect) { onToggleSelect(item, index); return }
          ...
          >
            ...
            {!multiSelect && onMorePress ? (
              <MoreButton onPress={() => onMorePress(item, index)} />
            ) : null}
          </TouchableOpacity>
        )
      }}
...
const styles = StyleSheet.create({
  ...
  duration: { fontSize: 12, minWidth: 40, textAlign: 'right', marginRight: 4 },
